...
- Monitor session progress and flag issues
- Suggest which tickets to work on next based on priority and context
...
export function startOrchestrator(): boolean {
  ...
  let initialResolve: ((value: string) => void) | null = null;

  async function* userInputStream() {
    // First message is the initial greeting prompt
    yield {
      type: "user" as const,
